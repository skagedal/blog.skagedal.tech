...
But it's difficult to argue about aesthetics. Some of it is of course just sort of aribtrary – for sure my taste in programming syntax has evolved over time, just as my taste in music has –  but  but I tend to think that the vocabulary of a system like HTML is intrinsically optimized to the shape of the syntax it has evolved within. A number of things immediately stand out as weird in the example above – like the plus sign that is used to generate a text node, and the fact that you can't naturally inline markup in a line of text. 
